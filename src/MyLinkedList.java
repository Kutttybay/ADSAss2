public class MyLinkedList<T> implements MyList {
    ...
    */
    ...
    /*
    * Этот метод сортировка элементов , если размер меньше двух то она просто возвращает
    * Если же код пойдет дальше то использует два указателя, один из которых указывает на текущий узел, а другой - на узел, следующий за текущим.
    * Далее метод проходится по списку, сравнивая текущий элемент с каждым последующим элементом и меняя их местами, если текущий элемент больше следующего.
    * Это происходит до тех пор, пока все элементы не будут проверены и отсортированы. В итоге, элементы списка будут отсортированы в порядке возрастания.
    */
    @Override
    public void sort() {
       if (size < 2){
